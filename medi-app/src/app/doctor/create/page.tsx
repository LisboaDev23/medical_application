...
export const DoctorCreate = () => {
    ...
    const addDoctor = async (e: any) => {
        //1 prevenir o evento padrão que é o de reload da página
        e.preventDefault();
        setError(null);

        //2 fazer validações do processo de login
        if(name != "" && login != ""
            && password != "" && medicalSpecialty != ""
            && medicalRegistration != "" && email != "" && phone != "") {

                //2.1 Definir o dado como um determinado objeto, é oq eu vou enviar
                const formData = {
                    name: name,
                    login: login,
                    password: password,
                ...
                //2.2 Fazer a requisição para a determinada url e configurar no método fetch oq ele irá fazer
                ...
                //atribuindo a requisição e pegando o conteudo dela em json
                const content = await add.json();
                //se houver o login, direcione o usuário para a página de inicio 
                if(content.login) {
                    router.push('/home');
                } else {
